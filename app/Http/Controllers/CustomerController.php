...
class CustomerController extends Controller
{
    ...
    public function __construct(User $user, UserMeta $usermeta, Post $post, PostMeta $postmeta, Setting $setting, Request $request)
    {
        ...
        $this->post_type = 'customer';
        $this->view      = 'app.customers';
    ...
    public function index()
    {
        ...
        if( Input::get('preview') ) {
            ...
            $pdf = PDF::loadView($this->view.'.reports.pricelist', $data);
            return $pdf->stream($this->post_type.'-pricelist.pdf');            
        }
    ...
    public function product()
    {
        ...
        $customer = json_decode($data['info']->customers, true);

        $data['info']['pid']            = $id;
        $data['info']['item']           = $id;
        $data['info']['customer_price'] = @$customer[$sid]['customer_price'];
        $data['info']['supplier_price'] = @$customer[$sid]['supplier_price'];
        ...
        $customer[$sid] = array(
            'customer'       => (string)$sid,
            'supplier_price' => @$info->normal_price,
            'customer_price' => Input::get('customer_price'),
        );


        $this->postmeta->update_meta($id, 'customers', json_encode($customer));
    ...
    {   
        ...
        $company_id = Input::get('company', $this->setting->get_setting('company'));
        ...
        $pdf = PDF::loadView($this->view.'.reports.soa', $data);
        return $pdf->stream($this->post_type.'-soa.pdf');            
    }    
