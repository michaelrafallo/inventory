...
<div class="portlet light bordered">
    ...
    <table class="table table-hover table-striped">
        ...
        <tbody>
            ...
            <tr>
                <td height="70">
                    <a href="{{ URL::route('app.customers.edit', [$postmeta->customer, 'tab' => 1]) }}" class="sbold">{{ $post->find($postmeta->customer)->post_title }}</a>
                    
                    <div class="btn-actions">                    
                        <div class="actions">                    
                        @if( Input::get('type') == 'trash')

                            @if( App\Permission::has_access('sales-orders', ['trash_restore']) )
                        ...
                        @else
                            ...
                            <a href="{{ URL::route($view.'.edit', [$row->id, 'preview' => 1]) }}" class="btn-preview btn btn-xs uppercase margin-top-10">Receipt</a>
                            @endif

                            @if( App\Permission::has_access('sales-orders', ['report']) )
                            <a href="{{ URL::route('app.customers.soa', [$postmeta->customer, 'soid' => $row->id, 'post_status' => $row->post_status]) }}" class="btn-xs btn btn-preview margin-top-10"> Invoice</a>
                            @endif

                            @if( App\Permission::has_access('sales-orders', ['trash_restore']) )
                                @if($row->post_status == 'unpaid' && $postmeta->delivered == 'undelivered')
                                <a href="#" class="popup btn btn-xs uppercase margin-top-10"
                ...
                <td> 
                    ...
                    @if(@$postmeta->si_no)
                    <br>SI NO. : {{ @$postmeta->si_no }}
                    @endif

                    @if(@$postmeta->dr_no)
                    <br>DR NO. : {{ @$postmeta->dr_no }}
                    @endif
                    </small>
                </td>
                ...
                <td class="text-center">{{ status_ico($postmeta->delivered) }}<br> {{ status_ico($row->post_status) }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>        
