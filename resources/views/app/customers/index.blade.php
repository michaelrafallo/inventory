        ...
        <tbody>
            ...
            <tr>
                <td>
                    ...
                    <div class="btn-actions margin-top-10">                    
                        <div class="actions">
                        @if( Input::get('type') == 'trash')

                            @if( App\Permission::has_access('customers', ['trash_restore']) )
                        ...
                        @else
                            ...
                            @if( App\Permission::has_access('customers', ['add_edit']) )
                            <a href="{{ URL::route($view.'.edit', [$row->id, 'tab' => 1]) }}" class="btn  btn-xs uppercase">Edit</a>
                            @endif

                            @if( App\Permission::has_access('customers', ['report']) )
                            <a href="{{ URL::route('app.customers.soa', $row->id) }}" class="btn-xs btn btn-preview">SOA</a>
                            @endif

                            @if( App\Permission::has_access('customers', ['report']) )
                            <a href="{{ URL::route($view.'.index', ['preview' => 'pricelist', 'post_id' => $row->id, query_vars()]) }}" class="btn-xs btn btn-preview"> Pricelist</a> 
                            @endif
